...
RCT::PingFinder::PingFinder()
    : gain(-1), sampling_rate(0), center_frequency(0), run_num(0), output_dir(""),
      test_data_path(""), ping_width_ms(36), ping_min_snr(4), ping_max_len_mult(1.5),
      ping_min_len_mult(0.75), sdr(nullptr), dsp(nullptr)
{
}
...
void RCT::PingFinder::start(void)
{
    ...
    else
    {
        ...
        else if (sdr_type == SDR_TYPE_FILE)
            sdr = new RCT::SDR_TEST(test_data_path, run_flag);
        else if (sdr_type == SDR_TYPE_GENERATOR)
            sdr = new RCT::SdrGenerator(gain, sampling_rate, center_frequency);
        else
    ...
    dsp->startProcessing(
        sdr_queue, sdr_queue_mutex, sdr_var, ping_queue, ping_queue_mutex, ping_var);
...
PYBIND11_MODULE(rct_dsp2, m)
{
    auto pf = py::class_<RCT::PingFinder>(
        ...
        "callbacks");
    pf.def(py::init(&RCT::PingFinder::create),
           "Creates a new PingFinder "
           "object.  This only creates the object, it does not initialize the "
           "underlying resources");
    pf.def("start",
           &RCT::PingFinder::start,
           "Starts the PingFinder.  This "
    ...
    pf.def("stop",
           &RCT::PingFinder::stop,
           "Stops the PingFinder.  This stops "
    ...
    pf.def_readwrite("gain",
                     &RCT::PingFinder::gain,
                     "Sets the internal gain "
                     "of the SDR");
    pf.def_readwrite("sampling_rate",
                     &RCT::PingFinder::sampling_rate,
                     "Sets "
                     "the sampling rate of the SDR");
    pf.def_readwrite("center_frequency",
                     &RCT::PingFinder::center_frequency,
                     "Sets the center frequency of the SDR");
    pf.def_readwrite("run_num",
                     &RCT::PingFinder::run_num,
                     "Set the current "
                     "run number");
    pf.def_readwrite("output_dir",
                     &RCT::PingFinder::output_dir,
                     "Sets the "
                     "output directory.  This is the directory in which the RAW_DATA* files "
                     "will be placed");
    pf.def_readwrite("enable_test_data",
                     &RCT::PingFinder::enable_test_data,
    ...
    pf.def_readwrite("test_data_path",
                     &RCT::PingFinder::test_data_path,
                     "Sets the test data directory.  If enable_test_data is set, "
                     "PingFinder will use the RAW_DATA* files in this directory.");
    pf.def_readwrite("ping_min_snr",
                     &RCT::PingFinder::ping_min_snr,
                     "Sets "
                     "the minimum ping SNR to be accepted");
    pf.def_readwrite("ping_width_ms",
                     &RCT::PingFinder::ping_width_ms,
                     "Sets "
                     "the nominal ping width in milliseconds");
    pf.def_readwrite("ping_max_len_mult",
                     &RCT::PingFinder::ping_max_len_mult,
                     "Sets the max multiplier of the ping width");
    pf.def_readwrite("ping_min_len_mult",
                     &RCT::PingFinder::ping_min_len_mult,
                     "Sets the min multipler of the ping width");
    pf.def_readwrite(
        "target_frequencies", &RCT::PingFinder::target_frequencies, "Sets the target frequencies");
    pf.def_readwrite("sdr_type", &RCT::PingFinder::sdr_type, "Sets the target SDR");
