...
namespace RCT
{
    ...
    HackRF::HackRF(double gain, uint64_t rate, uint64_t freq)
        : device(0), total_rx_samples(0), sampling_rate(rate)
    {
        int retval;
        ...
        {
            syslog(
                LOG_ERR, "Failed to set LNA gain: %s\n", hackrf_error_name((hackrf_error)retval));
        ...
        {
            syslog(
                LOG_ERR, "Failed to set VGA gain: %s\n", hackrf_error_name((hackrf_error)retval));
    ...
    HackRF::~HackRF()
    {
        ...
        {
            syslog(
                LOG_ERR, "Unable to close HackRF: %s\n", hackrf_error_name((hackrf_error)retval));
        }
        hackrf_exit();
    }

    void HackRF::startStreaming(std::queue<std::complex<double> *> &queue,
                                std::mutex &mutex,
                                std::condition_variable &cond_var)
    {
        ...
        {
            syslog(
                LOG_ERR, "Failed to start receive: %s\n", hackrf_error_name((hackrf_error)retval));
    ...
    void HackRF::rx_callback(hackrf_transfer *pTransfer)
    {
        ...
        {
            output_buffer[idx] =
                std::complex<double>(pData[idx * 2] / 128.0, pData[idx * 2 + 1] / 128.0);
        }
