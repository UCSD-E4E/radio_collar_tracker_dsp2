...
namespace RCT
{
    SDR_TEST::SDR_TEST(std::string input_dir, volatile bool &program_run)
        : _input_dir(input_dir), _stream(), _files(), _buffer_size(2048), _sampling_freq(4000000),
          _p_run(program_run)
    {
        glob_t glob_output;
                                  ...
                                  std::condition_variable &input_cv)
    {
        ...
        _thread = new std::thread(&SDR_TEST::_process,
                                  this,
                                  std::ref(input_queue),
                                  std::ref(input_mutex),
                                  std::ref(input_cv));
                            ...
                            std::condition_variable &data_cv)
    {
        ...
        std::size_t ms_per_buffer = (std::size_t)((double)_buffer_size / _sampling_freq * 1000);
        ...
        for (std::size_t i = 0; i < _files.size() && _run; i++)
        {
            ...
            for (std::size_t j = 0; j < num_samples / _buffer_size && _run; j++)
            {
                ...
                _stream.read((char *)buffer, sizeof(std::int16_t) * 2 * _buffer_size);
                for (std::size_t k = 0; k < _buffer_size && _run; k++)
                {
                    raw_buffer[k] =
                        std::complex<double>(buffer[2 * k] / 4096.0, buffer[2 * k + 1] / 4096.0);
