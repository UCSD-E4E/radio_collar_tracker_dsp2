...
namespace RCT
{
    USRP::USRP(double gain, long int rate, long int freq)
        : device_args(""), subdev("A:A"), ant("RX2"), ref("internal"), cpu_format("fc64"),
          wire_format("sc16"), channel{0}, if_gain(gain), rx_rate(rate), rx_freq(freq), _start_ms(0)
    {
        double value{0};
        ...
        if (error != UHD_ERROR_NONE)
        {
            ...
            throw std::runtime_error(uhd_strerror(error).c_str());
        }
    ...
    void USRP::startStreaming(std::queue<std::complex<double> *> &queue,
                              std::mutex &mutex,
    ...
    void USRP::streamer()
    {
        ...
        while (run)
        {
            std::complex<double> *raw_buffer =
                new std::complex<double>[this->getRxBufferSize() * 2];

            gettimeofday(&starttime, NULL);

            // get a buffer of data from rx_streamer and put in raw_buffer
            uhd_rx_streamer_recv(rx_streamer,
                                 (void **)&raw_buffer,
            ...
            // std::vector<std::complex<double>>& double_data = *(new
            // std::vector<std::complex<double>>()); double_data.resize(rx_buffer_size);
            if (_start_ms == 0)
            {
...
#ifdef DEBUG
        std::cout << time_inc / time_count << " us for recording " << rx_buffer_size << " samples"
                  << std::endl;
        std::cout << time_reset / time_count << " us for reset " << rx_buffer_size << " samples"
                  << std::endl;
        std::cout << ((time_inc / time_count + time_inc1 / time_count) / 1e6) /
                         ((double)rx_buffer_size / rx_rate) * 100
                  << "% of record time" << std::endl;
        ...
        std::cout << "SDR Received " << total_samples << " samples, "
                  << (double)total_samples / rx_rate << " seconds of data" << std::endl;
    ...
    USRP::USRP()
        : device_args("num_recv_frames=512"), subdev("A:A"), ant("RX2"), ref("internal"),
          cpu_format("sc16"), wire_format("sc16"), channel{0}, if_gain(0), rx_rate(2000000),
          rx_freq{0}
    {
        uhd_error error = uhd_usrp_make(&usrp, device_args.c_str());
        if (error != UHD_ERROR_NONE)
