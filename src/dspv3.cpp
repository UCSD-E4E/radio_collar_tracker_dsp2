...
namespace RCT
{
    DSP_V3::DSP_V3(const std::size_t sampling_freq,
        ...
        : target_freqs(target_freqs), nFreqs(target_freqs.size()), s_freq{sampling_freq},
          c_freq{center_freq}, input_block_size(input_block_size),
          FRAMES_PER_FILE(SAMPLES_PER_FILE / input_block_size),
          fixed_point_scalar(pow(2, bit_depth - 1)), input_hwm(0), power_hwm(0),
          _output_fmt(nullptr)
    {
        ...
        for (auto it = target_freqs.begin(); it != target_freqs.end(); it++)
        {
            ...
            {
                // upper half
                std::size_t target_bin =
                    ((double)(*it - center_freq) / (sampling_freq / 2)) * FFT_LEN / 2;
            ...
            else
            {
                ...
                std::size_t target_bin =
                    FFT_LEN +
                    ((double)(*it - (double)center_freq) / (sampling_freq / 2)) * FFT_LEN / 2;
                target_bins.push_back(target_bin);
#ifdef DEBUG
                _ostr1 << *it << ", " << target_bin << std::endl;
#endif
        ...
        _unpack_fft_plan =
            fftw_plan_dft_1d(FFT_LEN, _unpack_fft_in, _unpack_fft_out, FFTW_FORWARD, FFTW_MEASURE);
    ...
    void DSP_V3::startProcessing(std::queue<std::complex<double> *> &i_q,
                                 std::mutex &i_m,
                                 std::condition_variable &i_v,
                                 std::queue<PingPtr> &o_q,
                                 std::mutex &o_m,
                                 std::condition_variable &o_v)
    {
        ...
        _thread =
            new std::thread(&DSP_V3::_unpack, this, std::ref(i_q), std::ref(i_m), std::ref(i_v));
        _c_thread = new std::thread(&DSP_V3::classify,
                                    this,
                                    std::ref(_c_q),
                                    std::ref(_c_m),
                                    std::ref(_c_v),
    ...
    DSP_V3::max(boost::circular_buffer<std::shared_ptr<std::vector<double>>> &sig) const
    {
    ...
    std::vector<double> *
    DSP_V3::sig_median(boost::circular_buffer<std::shared_ptr<std::vector<double>>> &sig) const
    {

        boost::accumulators::
            accumulator_set<double, boost::accumulators::features<boost::accumulators::tag::median>>
                acc[nFreqs];
        for (auto it = sig.begin(); it != sig.end(); it++)
        {
    ...
    std::shared_ptr<std::vector<bool>> DSP_V3::compare(const std::vector<double> &data,
                                                       const std::vector<double> &threshold,
                                                       const double min_snr) const
    {
        ...
        boost::circular_buffer<std::shared_ptr<std::vector<bool>>>::iterator it) const
    {
        ...
        std::shared_ptr<std::set<std::size_t>> retval_ptr =
            std::make_shared<std::set<std::size_t>>();
    ...
    bool is_rising_edge(boost::circular_buffer<std::shared_ptr<std::vector<bool>>>::iterator it,
                        std::size_t i)
    {
    ...
    void DSP_V3::classify(std::queue<std::shared_ptr<std::vector<double>>> &i_q,
                          std::mutex &i_m,
                          std::condition_variable &i_v,
                          std::queue<PingPtr> &o_q,
                          std::mutex &o_m,
...
// std::ofstream _ostr6{"classifier_test.log"};
        ...
        while (_run || !i_q.empty())
        {
            ...
            if (!i_q.empty())
            {
                ...
                std::shared_ptr<std::set<std::size_t>> edge_ptr =
                    has_falling_edge(id_signal.end() - 1);
                if (!edge_ptr->empty())
                {
                    ...
                    for (auto it = edges.begin(); it != edges.end(); it++)
                    {
                        std::size_t pulse_width =
                            get_pulse_width(id_signal.end() - 1, id_signal.begin(), *it);
                        ...
                        double ping_start_ms =
                            (std::size_t)((signal_idx - pulse_width) * _ms_per_sample);
                        ...
                        PingPtr ping = std::make_shared<Ping>(
                            ping_start_ms + time_start_ms, amplitude, target_freqs[*it]);

                        {
                            ...
                            o_v.notify_all();
#ifdef DEBUG
                            std::cout << "Ping " << out_count << " at " << std::setprecision(3)
                                      << ping->time_ms << "ms " << ", amplitude " << amplitude
                                      << ", threshold: " << threshold[*it] + MIN_SNR
                                      << ", width: " << pulse_width * _ms_per_sample
                                      << ", freq: " << std::fixed << std::setprecision(0)
                                      << target_freqs[*it] << ", idx: " << *it
...
// _ostr6.close();
        ...
        delete threshold_ptr;
        std::cout << "Classifier received " << sample_counter << " samples, estimated "
                  << sample_counter * _ms_per_sample / 1e3 << " s" << std::endl;
    }
    ...
    void DSP_V3::_unpack(std::queue<std::complex<double> *> &i_q,
                         std::mutex &i_m,
                         std::condition_variable &i_v)
    {
        ...
        std::cout << "Unpack received " << sample_counter << " samples, estimated "
                  << (double)sample_counter / s_freq << " seconds of data" << std::endl;
        std::cout << "Unpack ran " << fft_counter << " ffts" << std::endl;
        std::cout << "Unpack wrote " << write_counter / 1024 / 1024 << " MB, estimated "
                  << (double)write_counter / 4 / s_freq << " seconds of data" << std::endl;
